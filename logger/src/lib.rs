...
lazy_static! {
    ...
    pub static ref LOGGER: &'static Logger = {
        set_logger(_LOGGER_INNER.deref()).expect("Failed to set logger: already initialized");
        _LOGGER_INNER.deref()
    };
}
    ...
    fn create_prefix(&self, record: &Record) -> String {
...

        {
            ...
            let id_guard = self
                .instance_id
                .read()
                .expect("Failed to read instance ID: poisoned lock");
    ...
    pub fn init(
...

        {
            let mut id_guard = self
                .instance_id
                .write()
                .expect("Failed to set instance ID: poisoned lock");
        ...
        match self.level_info.writer() {
            x if x == Destination::Pipe as usize => {
                ...
                if let Err(_) = log_to_fifo(
                    ...
                    self.log_fifo_guard()
                        .as_mut()
                        .expect("Failed to write to fifo: poisoned lock"),
                ) {
