...
impl Vmm {
    ...
    fn start_vcpus(
        ...
        for cpu_id in 0..vcpu_count {
            ...
            let vcpu_exit_evt = self
                ...
                .i8042
                .lock()
                .expect("Failed to start VCPUs: poisoned i8042 lock")
                .get_eventfd_clone()
    ...
    fn register_events(&mut self) -> std::result::Result<(), StartMicrovmError> {
        ...
        let event_fd = self
            ...
            .i8042
            .lock()
            .expect("Failed to register events on the event fd: poisoned lock")
            .get_eventfd_clone()
            .map_err(|_| StartMicrovmError::EventFd)?;
    ...
    fn start_microvm(&mut self) -> std::result::Result<VmmData, VmmActionError> {
        ...
        self.shared_info
            .write()
            .expect("Failed to start microVM: couldn't write shared info: poisoned lock")
    ...
    fn is_instance_initialized(&self) -> bool {
        let instance_state = {
            // Use expect() to crash if the other thread poisoned this lock.
            let shared_info = self.shared_info.read().expect("Failed determine if instance is initialized: couldn't read shared info: poisoned lock");
                            ...
                            match stdin_lock.read_raw(&mut out[..]) {
                                ...
                                Ok(count) => {
                                    ...
                                    self.legacy_device_manager
                                        .stdio_serial
                                        .lock()
                                        .expect("Failed to process stdin event: poisoned lock")
