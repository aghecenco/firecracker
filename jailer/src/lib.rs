    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            ReadLine(ref path, ref err) => {
                write!(f, "Failed to read line from {:?}: error {:?}", path, err)
            }
            ReadToString(ref path, ref err) => write!(
                f,
                "Failed to read file {:?} into a string: error {:?}",
                path, err
            ),
            RegEx(ref err) => write!(f, "Regex failed: {:?}", err),
            RmOldRootDir(ref err) => write!(f, "Failed to remove old jail root directory: {}", err),
            SeccompLevel(ref err) => write!(f, "Failed to parse seccomp level: error {:?}", err),
            SetCurrentDir(ref err) => {
                write!(f, "Failed to change current directory: error {:?}", err)
            }
            ...
            UnixListener(ref err) => {
                write!(f, "Failed to bind to the Unix socket: error {:?}", err)
            }
            ...
            Write(ref path, ref err) => write!(f, "Failed to write to {:?}: error {:?}", path, err),
        }
    }
}
...
mod tests {
    ...
    fn test_error_display() {
        ...
        let err2_str = "Os { code: 2, kind: NotFound, message: \"No such file or directory\" }";
        ...
        assert_eq!(
            format!("{}", Error::Chroot(err42.clone())),
            "Failed to chroot into the new jail folder: Errno 42",
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!(
                "Failed to copy \"/foo/bar\" to \"/foo\": error {}",
                err2_str
            )
        );
        assert_eq!(
            ...
            format!("Failed to create directory \"/foo\": error {}", err2_str)
        );
        assert_eq!(
            format!(
                "{}",
        ...
        assert_eq!(
            ...
            format!("Failed to exec into Firecracker: error {}", err2_str)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!("Failed to open file \"/foo/bar\": error {}", err2_str)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!("Failed to read line from \"/foo/bar\": error {}", err2_str)
        );
        assert_eq!(
            format!(
                "{}",
            ...
            format!(
                "Failed to read file \"/foo/bar\" into a string: error {}",
                err2_str
            )
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "Failed to parse seccomp level: error ParseIntError { kind: Overflow }",
        );
        assert_eq!(
            format!("{}", Error::SetCurrentDir(io::Error::from_raw_os_error(2))),
            format!("Failed to change current directory: error {}", err2_str),
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!("Failed to bind to the Unix socket: error {}", err2_str),
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!("Failed to write to \"/foo/bar\": error {}", err2_str),
        );
    }
}
